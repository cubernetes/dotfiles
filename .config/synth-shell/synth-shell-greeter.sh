...
printInfoCPUTemp()
{
	if ( which sensors > /dev/null 2>&1 && sensors > /dev/null 2>&1); then
		...
		#             sed -n 's/^.*crit = +\(.*\) [[CF]]*[ \t]*.*/\1/p')
                ...
                sensors > /tmp/.sensoroutput
                local current=$(grep Tctl /tmp/.sensoroutput | awk '{ print $2 }' | cut -c2-)
                local max=$(grep crit /tmp/.sensoroutput | awk '{ print $3 }' | cut -c2-)
                local high=$(grep Composite /tmp/.sensoroutput | sed -e 's/.*high = //g' | awk '{ print $1 }' | cut -c2-)
		rm -f /tmp/.sensoroutput
